...
- **コア crate は window の生成・管理をしない。**
  windowing はイベントループ・DPI・入力と不可分の別ドメインであり、コアに入れるとヘッドレス利用に windowing 依存が付く。
  コアは surface 生成の入力として自身が定義する中立な native handle 型(Win32 / Wayland)を受け取るだけにする(詳細は [0009](0009_surface-swapchain-present.md))。
  この非目標はコア crate のスコープであり、windowing(HDR / 広色域の出力設定を含む)は同リポジトリの別クレート orvk-window が担う([0012](0012_windowing-crate.md))。
- **シェーダのコンパイルをしない。**
...
orvk は、Rust の **Vulkan 専用ラッパーライブラリ** である。
...
1. **コアライブラリは単一 crate**。記録語彙・レジストリ・検証は feature なしでコンパイル可能とし、Vulkan 実行(instance / device 生成、submit)は device feature(既定 on)でゲートする。これにより、GPU の無い CI でも記録と検証のテストが回る([0006](0006_device-and-execution-model.md))。リポジトリには別プロダクトのクレート(windowing の [0012](0012_windowing-crate.md) 等)を workspace として並置してよいが、コアはそれらに依存しない。
